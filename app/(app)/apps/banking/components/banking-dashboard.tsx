...
import { RefreshCw, Trash2, Plus, Download, Building2, AlertCircle, CheckCircle2 } from "lucide-react"
...
export default function BankingDashboard({ accounts: initialAccounts }: { accounts: SafeBankAccount[] }) {
  ...
  async function handleDatevExport() {
    const result = await exportDatevAction()
        ...
        <div className="flex gap-2">
          ...
          {accounts.length > 0 && (
            <Button variant="outline" onClick={handleDatevExport}>
              <Download className="w-4 h-4 mr-1" /> DATEV Export
            </Button>
          )}
        </div>
      )}
